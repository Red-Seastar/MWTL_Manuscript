...
\title{A Reconfigurable Wireless Power Transfer System With Enhanced Spatial Freedom}
...
{Hong \MakeLowercase{\textit{et al.}}: A Reconfigurable Wireless Power Transfer System With Enhanced Spatial Freedom}

\maketitle

\begin{abstract}
In this letter, a reconfigurable wireless power transfer (WPT) system is proposed and demonstrated at 13.56~MHz. The transmitter consists of three single-layer triangular double-D coils arranged as a hexagonal array and driven by a relay-based coil-selection network. Measurements show that the proposed system achieves stable power transfer over the full azimuth range of 0$^\circ$--360$^\circ$, with an average power transfer efficiency (PTE) of 66.4\%. Additional measurements confirm that the PTE remains above 60\% under elevation variation and above 65\% during azimuthal rotation at different elevation angles (30$^\circ$ and 60$^\circ$). Lateral-position measurements show PTE above 50\% at large offsets. These results demonstrate that the proposed system can maintain stable wireless power transfer performance under receiver movement.
\end{abstract}

\begin{IEEEkeywords}
Wireless power transfer (WPT), magnetic resonance coupling (MRC), discrete coil selection, 2-D transmitter, power transfer efficiency.
\end{IEEEkeywords}
...
\IEEEPARstart{N}{ear-field} wireless power transfer (WPT) has been widely studied for contactless energy delivery in consumer electronics \cite{consumer}, wearable devices \cite{wearable}, biomedical systems \cite{bio}, and electric-vehicle applications \cite{ev}. In practical scenarios, the relative position and angular placement between transmitter (Tx) and receiver (Rx) may vary and cause coupling degradation. To maintain stable power delivery, the design of WPT systems with improved spatial freedom remains a long-standing challenge.

Many studies have improved position freedom by modifying the coil geometry \cite{geo1,geo3,geo4} or arrangement \cite{ar3,ar4}. These methods can alleviate coupling degradation when the receiver is laterally displaced over the transmitter surface. However, most are developed assuming parallel transmitter and receiver alignment, making them unable to accommodate angular variations from receiver rotation or elevation changes.

A common approach to improving angular freedom is to employ three-dimensional coil structures. Three-dimensional receivers can effectively maintain coupling in different angular positions \cite{3drx1,3drx3,3drx5}, but their volumetric structure limits their suitability for compact devices. Alternatively, three-dimensional transmitters \cite{comp2} or enclosing transmitters \cite{bowl,3D} can also enhance spatial freedom while allowing the receiver to remain 2-D. However, enclosing transmitter structures may obstruct the device during charging, making them less suitable for charge-while-use applications.

More recently, WPT systems with enhanced angular freedom have also been realized using only 2-D coil structures. For example, \cite{consu} generates a rotating magnetic field by synthesizing a 3-D magnetic field. In \cite{multi}, magnetic field shaping is achieved through closed-loop phase control. These methods can effectively address angular variation without using \mbox{3-D} coils, but often require more complex implementations that involve multiple excitation channels or multilayer coil structures.

In this letter, a reconfigurable WPT system with enhanced spatial freedom is proposed. With its simple, single-layer, and open transmitter structure, the proposed system can maintain effective power transfer under both azimuthal rotation and elevation variation.
...
The transmitter topology of the proposed WPT system is shown in Fig.~\ref{fig1}. It consists of three triangular double-D (DD) coils and a relay network for discrete coil selection (DCS). A hexagonal source loop is placed at the bottom of the transmitter and is connected to the three coils through normally open relays (Omron G6L-1F). To energize a particular coil, the two relays associated with that coil are closed, while the others remain open, so that only one Tx coil is activated at a time. The transmitter is driven by a single-channel excitation source (i.e., one port).
...
The Rx used in this work is a 2-D circular coil with a diameter of 78.4~mm. The Rx coil has nine turns, with an inner radius of 20.0~mm, an outer radius of 38.2~mm, a trace width of 1.4~mm, and a trace gap of 0.7~mm. Both the Tx and Rx coils are fabricated on a 1.0~mm thick FR4 substrate.

The proposed WPT system is designed to tolerate both azimuthal rotation and elevation variation of the Rx. During azimuthal rotation, different Tx coils are activated in different angular regions to maintain effective power transfer. Specifically, coil~1 covers $120^\circ$--$180^\circ$ and $300^\circ$--$360^\circ$, coil~2 covers $60^\circ$--$120^\circ$ and $240^\circ$--$300^\circ$, and coil~3 covers $0^\circ$--$60^\circ$ and $180^\circ$--$240^\circ$. For elevation variation, the DD coil can naturally maintain effective coupling, because its magnetic field contains components intersecting the Rx plane when the Rx is tilted.
...
\begin{equation}
\label{deqn_ex1}
\eta = \frac{|S_{21}|^2}{1-|S_{11}|^2}\times100\%
\end{equation}
...
Full-wave simulations of the hexagonal array transmitter were conducted in CST Studio Suite to evaluate the magnetic field distribution. At a height of $z=5$~mm above the transmitter plane, the $z$-component of the magnetic field was recorded for each coil when energized individually, as shown in Fig.~\ref{fig4}. The two spirals within each triangular double-D (DD) coil produced comparable field strengths in opposite directions, and all three cases exhibited clear N--S polarity, which preliminarily verified the intended behavior of the DD coils. The field maps also indicate that the deactivated coils do not noticeably distort the active-coil field distribution.

An azimuthal rotation experiment was performed with the vertical Rx setup ($\phi=90^\circ$). As shown in Fig.~\ref{fig6}, by selecting among the three Tx coils, the proposed system maintains effective power transfer over the full azimuth range of $\theta=0^\circ$ to $360^\circ$. The measured average PTE is 66.4\%, with a maximum of 68.3\% and a minimum of 63.4\%.
...
Additionally, azimuthal rotation was investigated at different elevation angles, as shown in Fig.~\ref{fig7}(b). The PTE was measured over an azimuthal range of $\theta=0^\circ$ to $180^\circ$ in $15^\circ$ increments at $\phi=30^\circ$ and $\phi=60^\circ$. In both cases, the PTE remains above 65\% over the entire measured rotation range. The average measured PTE is 81.6\% at $\phi=30^\circ$ and 70.2\% at $\phi=60^\circ$, while the peak PTE reaches 85.6\% at $\phi=30^\circ$. These results indicate that the proposed WPT system can maintain effective power transfer when azimuthal rotation and elevation variation occur simultaneously.
...
\caption{Comparison with other WPT systems}
\label{tab1}

\begin{threeparttable}
\begin{tabular}{cccccc}
\hline
\textbf{Ref.} & \cite{comp2} & \cite{3D} & \cite{consu} & \cite{hybrid}  & \textbf{This work} \\
\hline
\hline
Year  & 2020 & 2023 & 2024 & 2024 & \textbf{2026} \\
\hline
Freq. (MHz) & 13.56 & 6.78 & 6.78 & 6.78 & \textbf{13.56} \\
\hline
Dim. Tx \,$\mid$\,Rx\tnote{1} & 3\,$\mid$\,2 & 3\,$\mid$\,2 & 2\,$\mid$\,2 & 2\,$\mid$\,2 & \textbf{2\,$\mid$\,2} \\
\hline
Method & 3-D coil & 3-D coil & 3-D H-field & IMN\tnote{2} & \textbf{DCS} \\
Excitation & 1 & 1 & 3 & 1 & \textbf{1} \\
\hline
$\Delta \eta$, Az.\tnote{3} & - & 30\% & 15.4\% & 5\% & \textbf{7.2\%} \\
\hline
Max. $\eta$ & 83.6\% & 71.6\% & 78\% & 80\% & \textbf{85.6\%} \\
\hline
\end{tabular}
\vspace{0.5em}
...
\begin{tablenotes}
\footnotesize
\item[1] Dim. Tx\,$\mid$\,Rx represents the structural dimensionality of the WPT systems, where 3\,$\mid$\,2 means 3-D transmitter and 2-D receiver.
\item[2] IMN denotes the impedance-matching network.
\item[3] $\Delta \eta$, Az. represents the PTE variation over the azimuthal rotation range, calculated as $\Delta \eta = \frac{\eta_{\max}-\eta_{\min}}{\eta_{\max}}\times100\%$.
...
Finally, Table~\ref{tab1} compares the proposed system with recent WPT systems designed to improve spatial freedom. The proposed system provides azimuthal and elevation tolerance comparable to those of other recent works, while using a relatively simple strategy and transmitter structure. Its azimuthal PTE variation of 7.2\% also indicates high stability during receiver rotation. For example, \cite{consu} employs three excitation channels with independent current control to generate a 3-D rotating magnetic field. By contrast, the proposed system requires only a single excitation channel with a simple DCS network. In \cite{comp2}, a three-dimensional transmitter is used to improve elevation coverage; however, the system still exhibits a power null point and requires mechanical adjustment of the transmitter coil. A 3-D transmitter is also adopted in \cite{3D}, achieving both azimuthal and elevation tolerance. Nevertheless, the enclosed transmitter structure in \cite{3D} is more suitable for charging devices in idle states, whereas the proposed planar Tx is better suited to charge-while-use applications due to its open transmitter structure. Furthermore, \cite{hybrid} presents an elegant passive solution that utilizes a hybrid Tx with impedance-matching networks to achieve angular coverage. The hybrid Tx in \cite{hybrid} is a multilayer structure composed of a bow-tie coil, a circular coil, and a DD coil. By contrast, the proposed system uses a single-layer printed PCB Tx array, which has a more straightforward coil design while requiring a simple DCS network. 


\section{Conclusion}
A reconfigurable wireless power transfer system with enhanced spatial freedom has been demonstrated at 13.56~MHz. Experimental results verify that the proposed system can maintain stable power transfer over the full azimuth range, while also operating effectively under elevation variation. Lateral-position measurements further show effective power transfer at large offsets, with a center null caused by the DD field distribution. These results confirm that the proposed system provides an effective WPT solution with enhanced spatial freedom.
