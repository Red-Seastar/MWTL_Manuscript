...
\begin{abstract}
In this letter, a reconfigurable wireless power transfer (WPT) system with enhanced spatial freedom is proposed and demonstrated at 13.56~MHz. The transmitter consists of three single-layer triangular double-D coils arranged as a hexagonal array and driven by a relay-based coil-selection network. Measurements show that the proposed system achieves stable power transfer over the full azimuth range of 0$^\circ$--360$^\circ$, with an average power transfer efficiency (PTE) of 66.4\%. Additional measurements confirm that the PTE remains above 60\% under elevation variation and above 65\% during azimuthal rotation at different elevation angles (30$^\circ$ and 60$^\circ$). Lateral-position measurements show $>$50\% PTE at large offsets. These results demonstrate that the proposed system provides enhanced spatial freedom with a simple single-layer transmitter structure and a low-complexity discrete coil selection strategy.
\end{abstract}

\begin{IEEEkeywords}
Wireless power transfer (WPT), magnetic resonance coupling (MRC), spatial freedom, discrete coil selection, 2-D transmitter, power transfer efficiency.
\end{IEEEkeywords}
...
A common approach to improving angular freedom is to employ three-dimensional coil structures. Three-dimensional receivers can effectively maintain coupling in different angular positions \cite{3drx1,3drx3,3drx5}, but their volumetric structure limits their suitability for compact devices. Alternatively, three-dimensional transmitters can also enhance spatial freedom while allowing the receiver to remain 2-D \cite{comp2,bowl,3D}. However, enclosing transmitter structures may obstruct the device during charging, making them less suitable for charge-while-use applications.

More recently, WPT systems with enhanced angular freedom have also been realized with 2-D coil structures only. For example, \cite{consu} generates a rotating magnetic field by applying amplitude modulation. In \cite{multi}, magnetic field shaping is achieved through closed-loop phase control. These methods can effectively address angular variation without using 3D coils, but often require more complex implementations that involve multiple excitation channels or multilayer coil structures.
...
Finally, Table~\ref{tab1} compares the proposed system with recent WPT systems designed for improved spatial freedom. The proposed system provides azimuth and elevation coverage comparable to that of other recent works. However, other systems usually rely on more complex strategies. For example, \cite{consu} employs three excitation channels with amplitude modulation, and \cite{multi} adopts four-channel closed-loop phase control. These methods require multiple independent excitation channels to create rotating or targeted magnetic fields. By contrast, the proposed system requires only a single excitation channel with DCS.
...
\hline
Method & Switching & modulation & IMN\tnote{2} & phase-shift & \textbf{DCS} \\
Excitation & 1 & 3 & 1 & 4 & \textbf{1} \\
\hline
Peak $\eta$ & 71.6\% & 78\% & 80\% & 80.2\% & \textbf{85.6\%} \\
\hline
\end{tabular}
\vspace{0.5em}
...
In addition, \cite{3D} also adopts a mechanism similar to the proposed DCS strategy, but its 3-D Tx geometry is more suitable for charging devices in idle states, whereas the proposed 2-D Tx is better suited to charge-while-use applications due to its open transmitter structure. Furthermore, \cite{hybrid} presents an elegant passive solution that utilizes a hybrid Tx with impedance-matching networks to achieve angular coverage. The hybrid Tx in \cite{hybrid} is a multilayer structure composed of a bow-tie coil, a circular coil, and a DD coil. By contrast, the proposed system uses a single-layer printed Tx array, which has a more straightforward coil design while requiring a simple DCS network.
